...
**Search & download (custom-22):** the same accordion has a **🔎 Search & download from CivitAI** section. Type a name, pick the type (LoRA, Checkpoint, TextualInversion) and the base model to list first (SDXL 1.0 by default, Pony, Illustrious, NoobAI, SD 1.5), hit **Search**: every *version* is listed, with its base model, size, author, trigger words and a preview (NSFW excluded unless ticked). Only architectures Fooocus can load are requested from CivitAI (SD 1.4/1.5 incl. LCM/Hyper, SDXL 0.9/1.0 incl. Lightning/Turbo/Hyper/Distilled, Pony, Illustrious, NoobAI — not Pony V7, which is AuraFlow), so a page of results is not filled with Flux; tick **Show other architectures** to list everything, the rest flagged ⛔ (the fork hides them from its lists). ⚠ marks SD 1.x (refiner / SD 1.5 LoRA only). **Save into** proposes where to file it inside the first folder of that type (`path_loras`, `path_checkpoints`, `path_embeddings`): an *existing* base folder matched loosely on the result's base model (`_SDXL_1_0`, `SDXL`, `_Pony`, `_Illustrous`, `_SD_1.5`…), then an existing category folder from its CivitAI tags (`style`, `actor`, `helper`, `accelerator`, `slider`…), or `.nsfw` / `.nsfw_style` for an NSFW model; `Other` (else the root) when no folder fits the base. Pick another folder or type a new one (created, always inside the models folder); no category folder is ever created on its own. **Download** streams the file there — skipping it when a file with that name is already filed anywhere in the tree — checks it against the SHA256 published by CivitAI (a mismatch deletes it), never overwrites an existing file, then refreshes the model and LoRA lists so it is selectable right away. Files restricted to logged-in users need the API key saved above.
...
- **Update a plugin (custom-20):** Manager tab → *Updates* → **🔍 Check** lists the new commits of the plugin repo, **⬆ Update** fast-forwards to them — never over a file you modified inside the plugin (same guard as the boot update). Dependencies are reinstalled only when their `requirements` file changed (the torch step is never replayed); tick the box to force it. The install strategy is remembered and replayed. The Manager shows the installed commit of each plugin.
...
**Zero impact when off:** nothing is downloaded, nothing is injected, the module is never imported.
...
### 16. 📊 X/Y/Z Grid (comparison sheets)
**Where:** inside the **📋 Job Queue** panel, "X/Y/Z Grid" accordion (needs `job_queue.enabled`, ON by default).

**What it does:** pick 1 to 3 axes among CFG, Steps, Sampler, Scheduler, Sharpness, Checkpoint, LoRA 1 weight, **LoRA 1 name**, **LoRA 1 name+weight** and **Preset** (applies the whole preset per cell — model, LoRAs, CFG, sampler, styles, negative — while keeping your prompt and aspect ratio constant), plus **Prompt S/R** (a1111-style search & replace: first value is the term to find in your prompt, the rest are its replacements, quotes protect commas), give each a comma-separated value list, hit build. Every combination lands in the job queue as a regular job (1 image per cell, same seed everywhere so the comparison is honest). When the series finishes, an annotated sheet (512px thumbnails, X values as columns, Y as rows, one sheet per Z value) is saved to `outputs/xyz_grids/` and shown in the gallery. Individual full-res images stay in outputs and the Asset Browser as usual.

**Comparing LoRA epochs (custom-18):** the **LoRA 1 name** axis swaps the file in LoRA slot 1 while keeping the weight set in the panel — exactly what you want when the same LoRA was trained over several epochs (`sickOllie_e000010, sickOllie_e000020, sickOllie_e000030`) or re-uploaded as several CivitAI versions. Names are matched loosely: any unambiguous fragment works, so `e000020` is enough — no need to paste `subfolder\myLora_v3_e000020.safetensors`. An ambiguous or unknown fragment is rejected **when you click Build**, not 40 minutes into the series. Use `None` as a value for a LoRA-free control cell, and quotes if a filename contains a comma. **LoRA 1 name+weight** varies both at once, written `name:weight` (`myLora_e10:0.6, myLora_e20:0.8`) — handy when the best weight differs per epoch. Cell labels are trimmed from the *left*, so the `_e000020` suffix that tells your epochs apart always stays visible.
...
### 18. 🏷️ AI provenance on every image (EU AI Act, art. 50)
**Where:** automatic, on every saved image; readable in **Input Image → Metadata**.

**What it does:** every image Fooocus saves now declares, in a machine-readable way, that it was generated by AI: an XMP packet with the standard IPTC property `Iptc4xmpExt:DigitalSourceType = trainedAlgorithmicMedia` and `xmp:CreatorTool` (PNG iTXt chunk, JPEG APP1 segment, WEBP XMP chunk — X/Y/Z sheets included). It carries **no prompt and no parameter**, so it is written even when metadata saving is off. Optionally, an invisible **TrustMark** watermark (Adobe, open source) is embedded too: set `provenance.watermark: true` and `pip install trustmark` (runs on CPU; if the package is missing the image is still saved, with the XMP declaration).

**Reading it:** drop an image in the Metadata tab — the JSON gets a `provenance` section: IPTC declaration found or not, creator tool, C2PA manifest (with `pip install c2pa-python`), TrustMark watermark (with `trustmark`). The absence of a mark proves nothing (metadata stripped by a website, another tool, recompression): the tab never says "authentic" or "not AI".

**Config:** `provenance.enabled` (default `true`), `provenance.watermark` (default `false`), `provenance.watermark_id` (9 ASCII chars, default `Fooocus26`).
...
(Last tested - 2026 Sep 18 on the free T4 tier, Colab Python 3.13 with a Python 3.11 venv)
