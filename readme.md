...
- **🔎 Search box** — header input filters the current model tab by filename, trigger words, prompt, or relative path. Debounced 200 ms. Empties on tab switch. Bug from custom-8: the LoRA tab's search silently did nothing because `trigger_words` is an array and the filter callback threw a `TypeError` — fixed in **custom-8.7**.
- **⭐ Favorites + sort (model tabs)** — *custom-8.8*. Each card has a ☆ in the top-left; click it to star. Starred items get a gold outline and a `⭐ Favorites (N)` virtual entry appears at the top of the sidebar. Sort dropdown in the header (Name ↑↓, Modified ↑↓, Size ↑↓) — persisted per kind. Favorites are **always pinned at the top** regardless of sort. The star is also available in the lightbox caption. All state lives in `localStorage` — no backend involvement, survives Fooocus restarts.
...
- **🌐 Fetch missing N — bulk CivitAI** — *custom-8.9*. Header button that appears when at least one model across LoRAs / checkpoints / embeddings has a placeholder preview. Sequential batch with 1.5 s throttle, floating progress overlay (Succeeded / Skipped / Failed counters), Cancel button, polite auto-stop on 5 consecutive HARD failures (network or rate-limit only — "not on CivitAI" type messages are counted as skips and don't penalize). Reload after the batch picks up the new previews.
- **🗑️ Delete (to OS trash)** — *custom-8.10*. Red button in the lightbox caption of every tab (Outputs + 3 model tabs). Detailed `confirm()` lists the file, size, and sidecars that will follow it. Uses the **Windows Recycle Bin** by default via PowerShell + `Microsoft.VisualBasic.FileIO` — no pip dependency required. Falls back to `send2trash` if installed, then to permanent `os.remove` as a last resort (with a `[asset-browser] WARNING: hard-deleted` log line). For models: deletes the `.safetensors` + every sidecar preview + `.civitai.json`/`.json`/`.txt` metadata + indexer cache thumbs, then rebuilds the manifest. For outputs: image + `_thumb.jpg` sidecar + patches the day's manifest in-place.
- **CivitAI update check + Apply** — *custom-8.11*. Opening a LoRA / checkpoint / embedding in the lightbox fires an async check against CivitAI: same architecture + newer version → violet **🆕 Newer version available** banner; **different architecture** (typically the author migrated SDXL → Flux on the same page) → orange **⚠ Architecture change** banner with an extra `confirm()` gate before any apply. Click **📥 Update from CivitAI** to expand an inline panel with 3 modes: *Send old to Trash* (default, recoverable), *Keep old as .old* (renames the old file to `<stem>.old<ext>`), or *Keep as it* (old stays untouched, new downloaded as `<stem>.new<ext>`). Downloads stream in 1 MiB chunks (no RAM blowout on 7 GB checkpoints), use the CivitAI API key from `config.txt` for gated models, and have full roll-back on any failure. Per-file result is cached 24h in `civitai_cache/<file>.updatecheck.json` so re-opening the same item is instant.
...
| `modules/civitai_api.py` | **New** — CivitAI client, caching, consensus aggregation, model+embedding triggers. Adds `get_latest_version_for_model()` for the update detector (custom-8.11). |
...
| `modules/model_indexer.py` | **New** — Asset Browser model scanners (LoRAs / Checkpoints / Embeddings) + sidecar preview lookup + placeholder generation (custom-8). `delete_file_for()` with native Windows Recycle Bin via PowerShell (custom-8.10). `check_update_for()` + `apply_update_for()` for the CivitAI update detector with 24h cache and architecture-change recognition (custom-8.11). |
| `gallery_template/index.html` + `_assets/` | **New** — Asset Browser SPA + bundled PhotoSwipe v5 / Dynamic Caption / Deep Zoom (custom-8). LoRA search fix (custom-8.7), Favorites + sort (custom-8.8), CivitAI batch fetch overlay (custom-8.9), Delete button (custom-8.10), CivitAI update banner + inline apply panel (custom-8.11). |
| `launch.py` | Spawns Asset Browser model indexer in a daemon thread when enabled (custom-8) |
| `webui.py` | All fork UI: Save Preset, CivitAI / LoRA / Embeddings / Wildcards accordions, Aspect-for-Vary, Custom Resolution panel, Asset Browser accordion + link button. Hidden Gradio bridges `/run/ab_delete_file` (custom-8.10), `/run/ab_check_update` + `/run/ab_apply_update` (custom-8.11). Restart UI button moved from Settings tab to bottom of Advanced tab (custom-8.12). |
